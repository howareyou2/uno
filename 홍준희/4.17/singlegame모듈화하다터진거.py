...
def set_window(screen_width, screen_height):
    ...
    p6_rect = pygame.Rect((section2_width - p_width) / 2, p_height * 5 + p_spacing * 6, p_width * 0.789,
                          p_height * 0.87)
...
def playgame(screen_width,screen_height):
    screen = pygame.display.set_mode((screen_width, screen_height))
    # 카드뽑기 함수, top에서
    ...
    set_window(screen_width,screen_height)
    ...
    for player in players:
        for j in range(len(player)):
            ...
            if splitCard[0] == "BLACK":
                ...
                if splitCard[1] == "RERVERSE" or splitCard[1] == "SKIP" or splitCard[1] == "DRAW2":
                    score += 20
                else:
                    score += splitCard[1]
...
def start_game():
    ...
    # discard_group= pygame.sprite.RenderPlain(discards[-1])
    # discard_group.update(300,110)
    # discard_group.draw(screen)
