...
def story_map4():
    ...
    waterfall_rect.center = (450, 410)
    
    hae_rect = hae_image.get_rect()
    hae_rect.center = (650, 200)
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    ...
                    #오른쪽으로 이동할 때 60도 회전
                    char_img1 = pygame.transform.rotate(char_img1, 60)
                    char_img2 = pygame.transform.rotate(char_img2, 60)
                    char_img3 = pygame.transform.rotate(char_img3, 60)
                    char_img4 = pygame.transform.rotate(char_img4, 60)
                    char_img_list = [char_img1, char_img2, char_img3, char_img4]
                    char_width, char_height = (40, 40)
                    char_img_list = [pygame.transform.scale(img, (char_width, char_height)) for img in char_img_list]
        ...
        #defining hae_x, hae_y, hae_width, hae_height
        hae_x = 570
        hae_y = 160
        hae_width = 98
        hae_height = 77
        ...
        #캐릭터와 hae이 충돌하면 캐릭터의 위치가 멈추게 설정, hae위에 텍스트 출력
        if char_x + char_width > hae_x and char_x < hae_x + hae_width and char_y + char_height > hae_y and char_y < hae_y + hae_height:
            char_x -= move_x
            char_y -= move_y
            hae_text = font.render("Seongsan: ENTER!", True, (0, 0, 0))
            screen.blit(hae_text, (hae_x, hae_y - 20))
    
            #defining mouse_x, mouse_y
            mouse_x, mouse_y = pygame.mouse.get_pos()
    
            #waterfall이미지와 충돌 후 엔터를 누르면 startPage으로 넘어가게 설정
            if event.type == pygame.KEYDOWN:
